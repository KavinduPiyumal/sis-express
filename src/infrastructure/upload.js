  ...
  if (process.env.UPLOAD_DRIVER === "s3") {
    ...
    upload = multer({
      storage: multerS3({
        ...
        ...(process.env.S3_IS_PRE_SIGNED === "false"
          ? { acl: "public-read" }
          : {}),
        key: (req, file, cb) => {
          ...
          cb(
            null,
            `uploads/${userId}/${finalSubDir}/${fileId}-${Date.now()}-${
              file.originalname
            }`
          );
        },
      ...
      fileFilter: (req, file, cb) => {
        ...
        if (finalSubDir === "profileImages") {
          ...
          errorMessage =
            "Only .jpg, .jpeg, .png files are allowed for profile images!";
        } else if (finalSubDir === "assets" || finalSubDir === "payments") {
          ...
          errorMessage =
            "File type not supported. Allowed: PDF, Word, Excel, PowerPoint, Images, Text files";
        } else {
    ...
    upload = multer({
      ...
      fileFilter: (req, file, cb) => {
        ...
        if (finalSubDir === "profileImages") {
          ...
          finalSubDir === "assets" ||
          finalSubDir === "medicals" ||
          finalSubDir === "payments"
        ) {
          // For notice attachments and general assets, allow various document types
          allowedTypes = [
            "application/pdf",
